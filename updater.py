...
def write_bulk(devh, data, attempts=5):
    sucess = False
    for _ in range(attempts):
        try:
            ...
            print(e, e.value)
    if not sucess:
        raise Exception("too many tries, aborting...")

def read_bulk(devh, length, attempts=5):
    sucess = False
    for _ in range(attempts):
        try:
            ...
            devh.controlWrite(CTRL_OUT, usb1.REQUEST_SET_CONFIGURATION,
                              0x0305, 0, transfer, timeout=100)
            ...
            for offset in range(0, len(data), block_size):
                ...
                print(">>> USB: read_block (offset={}, length={})".format(
                    offset, length))
                data += devh.controlRead(CTRL_IN, usb1.REQUEST_CLEAR_FEATURE,
                                         0x0306, 0, length, timeout=2000)
                sleep(0.01)
            sucess = True
            break
        except usb1.USBError as e:
